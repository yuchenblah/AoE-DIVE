...
class PrunedLlamaSMoEBlock(nn.Module):
    ...
    # ------ AoE gate_proj gating ------
    def forward(self, x):
        flat_x = x.view(-1, x.shape[-1])  # (tokens, hidden_size)

        gate_acts = []
        for expert in self.experts:
            act = F.silu(expert.gate_proj(flat_x))
            score = torch.norm(act, dim=-1)  # (tokens,)
            gate_acts.append(score)
        logits = torch.stack(gate_acts, dim=-1)  # (tokens, num_experts)
    ...
    #     gate_acts = []
    #     for expert in self.experts:
    #         act = expert.up_proj(flat_x)  # (tokens, intermediate_size)
    #         score = torch.norm(act, dim=-1)  # (tokens,)
    #         gate_acts.append(score)

    #     logits = torch.stack(gate_acts, dim=-1)  # (tokens, num_experts)

    #     gating_weights, indices = torch.topk(logits, self.num_selects, dim=-1)
    #     gating_weights = F.softmax(gating_weights, dim=-1)
    ...
    #         results[batch_idx] += (
    #             gating_weights[batch_idx, nth_expert, None]
    #             * expert(flat_x[batch_idx])
    #         )
    ...
    #     flat_x = x.view(-1, x.shape[-1])  # (tokens, hidden)

    #     gate_acts = []

    #     for expert in self.experts:

    #         g = expert.gate_proj(flat_x)   # W_gate x
    #         u = expert.up_proj(flat_x)     # W_up x

    #         act = F.silu(g) * u            # σ(W_gate x) ⊙ W_up x
    ...
    #     gating_weights, indices = torch.topk(
    #         logits, self.num_selects, dim=-1
    #     )
    ...
    #         up = expert.up_proj(flat_x)
    #         gate = expert.gate_proj(flat_x)

    #         a_int = F.silu(gate) * up
    #         a_out = expert.down_proj(a_int)

    #         score = torch.norm(a_out, dim=-1)
    #         gate_acts.append(score)

    #     logits = torch.stack(gate_acts, dim=-1)
